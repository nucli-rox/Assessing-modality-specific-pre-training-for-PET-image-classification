...
class MIM(nn.Module):
    ...
    def _forward_shared(self, input_data):
        ...
        return preds, feats, patch_mask_long, mask
    ...
    def train_step(self, batch):
        ...
        preds, _, patch_mask_long, _ = self._forward_shared(input_data)
    ...
    def validation_step(self, batch):
        with torch.no_grad():
            ...
            preds, feats, patch_mask_long, voxel_mask_spatial = self._forward_shared(
                input_data
            )
...
def build_mae_in_model(cfg):
    ...
    model = MIM(net=net, patch_size=patch_size, intensitylog=intensitylog)
    print(f"Total model parameters: {model.get_params()['paramsM']:.2f}M")
    return model
